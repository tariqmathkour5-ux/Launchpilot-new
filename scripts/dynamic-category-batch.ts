/**
 * Dynamic Category Management System - Batch Processor
 * Processes a specific batch of tools (500 tools per batch)
 * 
 * Usage: npx ts-node --project tsconfig.ts-node.json scripts/dynamic-category-batch.ts <startBatch> <endBatch>
 * Example: npx ts-node --project tsconfig.ts-node.json scripts/dynamic-category-batch.ts 10 15
 */
...
/**
 * Sync predefined categories from JSON
 */
async function syncCategories(): Promise<void> {
  if (!fs.existsSync(CATEGORIES_PATH)) {
    console.log('  ⚠️ Categories JSON not found, skipping sync\n');
    return;
  }
  ...
  console.log(`  ✓ Synced ${categories.length} predefined categories\n`);
}
...
async function processBatch(tools: Tool[], batchNumber: number, totalBatches: number): Promise<{processed: number, categoriesCreated: number}> {
  ...
  for (const tool of tools) {
    try {
      ...
      if (processed % 100 === 0) {
        console.log(`  ⏳ Progress: ${processed}/${tools.length} tools processed`);
      }
    } catch (error: any) {
      console.error(`  ❌ Failed to process tool ${tool.slug}:`, error?.message || error);
    }
  }
...
async function main() {
  ...
  const endBatch = parseInt(args[1]);
  
  console.log('🚀 Starting Dynamic Category Management Batch Processor\n');
  console.log('=' .repeat(60));
  console.log(`Processing batches ${startBatch}${endBatch ? ' to ' + endBatch : ''} (${BATCH_SIZE} tools each)`);
  ...
  try {
    ...
    const startIdx = (startBatch - 1) * BATCH_SIZE;
    const endIdx = endBatch ? Math.min(endBatch * BATCH_SIZE, tools.length) : tools.length;
    const batchesToProcess = Math.ceil((endIdx - startIdx) / BATCH_SIZE);
    
    let totalProcessed = 0;
    let totalCategoriesCreated = 0;
    
    console.log(`\n🔄 Processing ${batchesToProcess} batch(es) starting from batch ${startBatch}...\n`);
    ...
    console.log(`   Batches completed: ${batchesToProcess}`);
    
  } catch (error: any) {
    console.error('\n❌ Error during category management:', error?.message || error);
    process.exit(1);
  } finally {
